<template>
  <div class="container p-4 text-white" style="background-color: #142442;">
      ...
      <div class="row gap-5 text-center" id="status_tracker">
        ...
        <div @click="filterByStatus('InProgress')" class="col rounded-4 p-3 button-like-div " >
          <h6>In Progress</h6>
          <h5>{{ inProgress }}</h5>
        </div>
        <div @click="filterByStatus('Approved')" class="col rounded-4 p-3 button-like-div " >
          <h6>Approved</h6>
          <h5>{{ approved }}</h5>
        </div>
        <div @click="filterByStatus('Rejected')" class="col rounded-4 p-3 button-like-div " >
          <h6>Rejected</h6>
          <h5>{{ rejected }}</h5>
        </div>
      </div>
        ...
        <table class="table text-white" border="0">
          ...
          <tbody>
            <tr v-for="item in data" :key="item.id">
              <th>
                <span style="font-weight: 500;">{{ item.formName }}</span><br>
                <span style="font-size: smaller; color: grey; font-weight: 400;">{{ item.company }}</span>
              </th>
              <td class="align-middle" :class="getStatusColour(item.status)">{{ getStatus(item.status) }}</td>

              <td class="align-middle" v-if="item.status == 'Approved'">
                <button class="rounded bg-white me-4 border-none" ><img  src="../assets/images/previewIcon.png" height="25"></button>
                <button class="rounded bg-white" ><img  src="../assets/images/downloadIcon.png" height="25"></button>
              </td>
              <td class="align-middle" v-else-if="item.status == 'Rejected'">
                <button class="rounded bg-white me-4" ><img  src="../assets/images/previewIcon.png" height="25"></button>
              </td>
              <td class="align-middle" v-else-if="item.status == 'Pending'">
                <button class="rounded bg-white me-4" ><img  src="../assets/images/previewIcon.png" height="25"></button>
